...
function App() {
  ...
  if (status) {
    ...
    const color_aspects = Object.keys(outputt).map(
      (x) =>
    ...
    //aspect pie chart
    const options_aspectsChart = {
      responsive: true,
      plugins: {
        title: { display: true, text: "Summary of Aspects detected" },
      },
    };
    let negativeAspects = Object.keys(outputt)
      .sort(function (a, b) {
        return outputt[b]["negative"] - outputt[a]["negative"];
      })
      .splice(0, 3);

    const color_analysis = negativeAspects.map(
      (x) =>
        `rgba(${Math.floor(Math.random() * 255)}, ${Math.floor(
          Math.random() * 255
        )}, ${Math.floor(Math.random() * 255)}, 1)`
    ...
    const aspectsChart = {
      labels: negativeAspects,
      datasets: [
        {
          label: "Count",
          data: [
            outputt[negativeAspects[0]]["negative"],
            outputt[negativeAspects[1]]["negative"],
            outputt[negativeAspects[2]]["negative"],
          ],
    ...
    analysis = (
      ...
      >
        ...
        >
          <div style={{ width: "28%" }}>
            <Pie data={emotionsChart} options={options_emotionsChart} />
          </div>
          <div style={{ width: "28%" }}>
            <Pie data={aspectsChart} options={options_aspectsChart} />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: 600 }}>
      ...
      >
        <p style={{ fontSize: 80, fontWeight: "bold", marginBottom: 5 }}>
          AI Challenge 2023
          {
            <p style={{ fontSize: 20, marginLeft: 30 }}>
              We provide meaning to your sentences
            </p>
          }
        </p>
      </div>
